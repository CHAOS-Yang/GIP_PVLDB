...
from newfile.grad_clipping import pruning
import optax
...
class Updater:
  ...
  def __init__(
      ...
      *,
      ...
      per_example_pruning_amount = 1,
      batch_pruning_amount = 1,
      batch_pruning_method = 'NoPruning',
  ):
    ...
    self.per_example_pruning_amount=per_example_pruning_amount
    self.batch_pruning_amount=batch_pruning_amount
    self.batch_pruning_method=batch_pruning_method
    ...
    if (clipping_norm in (float('inf'), None) and
      ...
      self.value_and_clipped_grad = functools.partial(
          ...
          pruning_fn=grad_clipping.pruning(pruning_amount=self.per_example_pruning_amount),
      )
  ...
  def batch_pruning_top_k(  
    ...
    def leaves_pruning(leaves):
      quantile=jnp.percentile(jnp.abs(jnp.array(leaves)), batch_pruning_amount)
      return jax.tree_map(lambda x: x * get_mask(x, quantile), leaves)
  
    def get_mask(x, quantile):
      return  jnp.where(jnp.abs(x) > quantile, jnp.zeros_like(x), jnp.ones_like(x))
  ...
  def batch_pruning_random(  
    ...
    def get_mask(x, quantile):
      new_key,subkey=jax.random.split(self.random_key)
      normal_sample=jax.random.normal(subkey)
      self.random_key=new_key
      return jnp.where(jax.random.uniform(self.random_key)> quantile, jnp.zeros_like(x), jnp.ones_like(x))
  ...
  def _pmapped_update(
    ...
    if self.batch_pruning_method == 'TopK':
      grads = self.batch_pruning_top_k(self.batch_pruning_amount)(grads)
    elif self.batch_pruning_method == 'Random':
      grads = self.batch_pruning_random(self.batch_pruning_amount)(grads)
    else:
